...
if [ ! -n "$BASH" ]; then
	...
	bash -c '. <('"$(command -v curl >/dev/null && echo 'curl -Ls' || echo 'wget -qO-')"' https://github.com/Isatau/zeferio/blob/master/install.sh) '"$*"
	exit $?
fi
...
runin() {
	...
	{ "$@" 2>>../zeferio-install.log || return $?; } | while read -r line; do
		spin
		printf "%s\n" "$line" >>../zeferio-install.log
	done
}
...
touch zeferio-install.log
if [ ! x"$SUDO_USER" = x"" ]; then
	chown "$SUDO_USER:" zeferio-install.log
fi
...
if [ -f ".setup_complete" ] || [ -d "zeferio" -a ! x"" = x"$DYNO" ]; then
	# If zeferio is already installed by this script, or its in Heroku and installed
	PYVER=""
...
echo "Installing..." >zeferio-install.log
...
if echo "$OSTYPE" | grep -qE '^linux-gnu.*' && [ -f '/etc/debian_version' ]; then
	...
	if [ ! "$(whoami)" = "root" ]; then
		...
		if command -v sudo >/dev/null; then
			...
			echo "Relaunching" >>zeferio-install.log
			sudo "$BASH" -c '. <('"$(command -v curl >/dev/null && echo 'curl -Ls' || echo 'wget -qO-')"' https://github.com/Isatau/zeferio/master/install.sh) '"$*"
			BLA::stop_loading_animation
			exit $?
		else
			PKGMGR="true"
	...
	endspin "Unrecognised OS. Please follow https://t.me/zeferio_talks"
	BLA::stop_loading_animation
	exit 1
fi
...
${SUDO_CMD}rm -rf Zeferio
# shellcheck disable=SC2086
runout ${SUDO_CMD}git clone https://github.com/Isatau/zeferio/ || {
...
rm -f ../zeferio-install.log
touch .setup_complete
BLA::stop_loading_animation
# shellcheck disable=SC2086,SC2015
${SUDO_CMD}"python$PYVER" -m zeferio "$@" || {
