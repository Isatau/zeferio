...
photo = io.BytesIO(
    requests.get(
        "https://github.com/Isatau/zeferio/raw/master/assets/bot_pfp.png"
    ).content
)
...
class TokenObtainment(InlineUnit):
    async def _create_bot(self) -> None:
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            if self._db.get("zeferio.inline", "custom_bot", False):
                username = self._db.get("zeferio.inline", "custom_bot").strip("@")
                username = f"@{username}"
                try:
            ...
            for msg in [
                f"👾 zeferio Userbot of {self._name}",
                username,
                "/setuserpic",
                username,
            ]:
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            for row in r.reply_markup.rows:
                for button in row.buttons:
                    if (
                        self._db.get("zeferio.inline", "custom_bot", False)
                        and self._db.get("zeferio.inline", "custom_bot", False)
                        != button.text.strip("@")
                    ):
                        continue
                    ...
                    if not self._db.get(
                        ...
                        "custom_bot",
                        False,
                    ) and not re.search(r"@zeferio_[0-9a-zA-Z]{6}_bot", button.text):
                        continue
                    ...
                    self._db.set("zeferio.inline", "bot_token", token)
                    self._token = token
    ...
    async def _dp_revoke_token(self, already_initialised: bool = True) -> None:
        ...
        self._db.set("zeferio.inline", "bot_token", None)
        self._token = None
        if already_initialised:
