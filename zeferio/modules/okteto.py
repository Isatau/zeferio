...
class OktetoMod(loader.Module):
    """Stuff related to Zeferio Okteto cloud installation"""
    ...
    async def _okteto_poller(self) -> None:
        ...
        while True:
            try:
                if not self._db.get("zeferio", "okteto_uri", False):
                    await asyncio.sleep(self._env_wait_interval)
                    continue

                uri = self._db.get("zeferio", "okteto_uri")
                current_queue = (
                    await self._client(
    ...
    async def watcher(self, message: Message) -> None:
        if (
            ...
            or not self._db.get("zeferio", "okteto_uri", False)
            or self._db.get("zeferio", "okteto_uri") not in message.raw_text
