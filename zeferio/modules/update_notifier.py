...
class UpdateNotifierMod(loader.Module):
    """Tracks latest Zeferio releases, and notifies you, if update is required"""

    strings = {
        "name": "UpdateNotifier",
        "update_required": "👾 <b>Zeferio Update available!</b>\n\nNew Zeferio version released.\n🔮 <b>Zeferio <s>{}</s> -> {}</b>\n\n{}",
        "more": "\n<i><b>🎥 And {} more...</b></i>",
    }
    ...
    async def client_ready(self, client, db) -> None:
        ...
        self._markup.add(
            InlineKeyboardButton(
                "🔄 Update",
                callback_data="zeferio_update",
            )
        )
        self._markup.add(
            InlineKeyboardButton(
                "🚫 Ignore",
                callback_data="zeferio_upd_ignore",
            )
        )
    ...
    async def update_callback_handler(self, call: CallbackQuery) -> None:
        ...
        @allow: sudo
        """
        if call.data not in {"zeferio_update", "zeferio_upd_ignore"}:
            return

        if call.data == "zeferio_upd_ignore":
