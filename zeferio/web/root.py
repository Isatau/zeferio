...
class Web:
    ...
    async def okteto(self, request):
        if any(cl[2].get("zeferio", "okteto_uri", False) for cl in self.client_data.values()):
            return web.Response(status=418)

        text = await request.text()

        for client_uid in self.client_data:
            self.client_data[client_uid][2].set("zeferio", "okteto_uri", text)
    ...
    async def finish_login(self, request):
        ...
        first_session = not bool(main.zeferio.clients)

        await main.zeferio.fetch_clients_from_web()
        ...
        if not first_session:
            ...
            for client in main.zeferio.clients:
    ...
    async def web_auth(self, request):
        ...
        session = f"zeferio_{utils.rand(16)}"
        ...
        if not await main.zeferio.wait_for_web_auth(token):
